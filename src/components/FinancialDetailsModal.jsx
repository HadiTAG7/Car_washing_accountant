...
const HEADER_META = {
  ...
  variable: { title: 'التكاليف المتغيرة والعمولات',         icon: Activity,       accent: 'text-rose-600 dark:text-rose-400',       accentBg: 'bg-rose-50 dark:bg-rose-500/15' },
...
function RevenueTable({ rows }) {
  if (!rows.length) return <EmptyState message="لا توجد غسلات مكتملة في هذا الشهر" />;
  const total = rows.reduce((s, r) => s + r.total, 0);
  return (
    <table className="w-full min-w-[640px] text-sm">
      <thead>
        <tr className="text-right text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase border-b border-slate-200 dark:border-slate-700">
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800/60 hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
...
function VariableTable({ rows }) {
  if (!rows.length) return <EmptyState message="لا توجد تكاليف متغيرة لهذا الشهر" />;
  const total = rows.reduce((s, r) => s + r.total, 0);
  return (
    <table className="w-full min-w-[640px] text-sm">
      <thead>
          ...
          <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800/60 hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
            <td className="py-3 px-4 whitespace-normal break-words font-medium text-slate-900 dark:text-slate-100">
              <div className="flex items-center gap-1.5">
                <span>{r.name}</span>
                {r.isVirtual && (
                  <span className="inline-flex items-center gap-1 bg-primary-50 dark:bg-primary-500/20 text-primary-700 dark:text-primary-300 border border-primary-100 dark:border-primary-500/40 text-[10px] font-bold px-1.5 py-0.5 rounded-md">
                    تلقائي
                  </span>
                )}
              </div>
            ...
            <td className="py-3 px-4 whitespace-nowrap">
              <span className="inline-flex text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-1 rounded-md">{r.category}</span>
            </td>
            <td className="py-3 px-4 whitespace-nowrap text-center text-slate-700 dark:text-slate-300 tabular-nums">{formatNumber(r.quantity)}</td>
...
function MonthlyTable({ rows }) {
  if (!rows.length) return <EmptyState message="لا توجد مصاريف شهرية ثابتة لهذا الشهر" />;
          ...
          <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800/60 hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
            <td className="py-3 px-4 whitespace-normal break-words font-medium text-slate-900 dark:text-slate-100">
              <div className="flex items-center gap-1.5">
                <span>{r.name}</span>
                {r.isOneTime && (
                  <span className="inline-flex items-center gap-1 bg-accent-50 dark:bg-accent-500/15 text-accent-700 dark:text-accent-300 border border-accent-100 dark:border-accent-500/40 text-[10px] font-bold px-1.5 py-0.5 rounded-md">
                    مرة واحدة
                  </span>
                )}
              </div>
...
function AnnualTable({ rows }) {
  if (!rows.length) return <EmptyState message="لا توجد مصاريف سنوية مسجلة" />;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        ...
        className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl mx-4 my-4 max-h-[92vh] overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800"
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-3 px-5 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40">
          <div className="flex items-center gap-3 min-w-0">
            <span className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${meta.accentBg} ${meta.accent}`}>
          ...
          <button
            type="button"
            onClick={onClose}
            className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors shrink-0"
            aria-label="إغلاق"
          >
            <X size={20} />
          </button>
